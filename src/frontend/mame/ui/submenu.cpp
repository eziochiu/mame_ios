	...
	, m_driver(drv)
{
	...
	for (option &sm_option : m_options)
	{
		...
		case option_type::EMU:
			...
			if (sm_option.entry != nullptr && ((sm_option.entry->type() == core_options::option_type::STRING) || (sm_option.entry->type() == core_options::option_type::PATH) || (sm_option.entry->type() == core_options::option_type::MULTIPATH)))
			{
